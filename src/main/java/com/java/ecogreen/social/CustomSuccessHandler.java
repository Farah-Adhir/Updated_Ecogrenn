...
public class CustomSuccessHandler implements AuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if (authentication.getPrincipal() instanceof DefaultOAuth2User) {
			...
			String email = userDetails.getAttribute("email") != null ? userDetails.getAttribute("email")
			...
			String username1 = userDetails.getAttribute("email") != null ? userDetails.getAttribute("email")
					: userDetails.getAttribute("admin") + "@gmail.com";
			if (userRepo.findByEmail(username1) == null) {
				Admin admin = new Admin();
				admin.setEmail(redirectUrl);
				admin.setName(userDetails.getAttribute("email") != null ? userDetails.getAttribute("email")
						: userDetails.getAttribute("admin"));
